...
import { createCostLimiter, hasCostLimits } from "../../../infra/cost-limiter.js";
    ...
    try {
        ...
        try {
            ...
            if (hasCostLimits(costLimitsCfg)) {
                ...
                costLimiter = createCostLimiter(costLimitsCfg, costSnapshot.sessionCost, costSnapshot.dailyCost);
            }
            ...
            try {
                ...
                if (costLimiter && !aborted) {
                    ...
                    if (lastMsg) {
                        const msgRecord = lastMsg;
                        const runUsage = normalizeUsage(msgRecord.usage ?? undefined);
                        if (runUsage) {
                            ...
                            if (runCost !== undefined) {
                                ...
                                if (result.exceeded) {
                                    ...
                                    if (action === "abort") {
                                        abortRun(false, new Error(`Cost limit exceeded: ${result.limitKind} ` +
                                            `($${result.currentValue?.toFixed(4)} >= $${result.limitValue?.toFixed(2)})`));
                                    }
                                    else {
                                        log.warn(`Cost limit warning: ${result.limitKind} ` +
                                            `($${result.currentValue?.toFixed(4)} >= $${result.limitValue?.toFixed(2)})`);
                                    }
                                }
                            }
